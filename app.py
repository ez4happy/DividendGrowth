                ...
                if len(cells) >= 7:
                    ...
                    if len(dt) == 10 and dt.replace('/', '').isdigit():
                        if latest_dt is None or dt > latest_dt:
                            latest_dt, latest_cells = dt, cells
...
def get_position_from_price(price, band_prices):
    if price is None or np.isnan(price) or not band_prices or len(band_prices) != 5:
        return np.nan
...
if os.path.exists(file_path):
    ...
    if stochastic_col is None:
        st.error("Stochastic 컬럼이 없습니다."); st.stop()

    # ROE 컬럼 확인
    roe_cols = [c for c in df.columns if 'ROE' in c and '평균' not in c and '최종' not in c]
    if len(roe_cols) < 3:
        st.error(f"ROE 컬럼이 3개 미만 입니다. 현재: {roe_cols}")
        st.stop()
    roe_cols = roe_cols[:3]
    ...
    for idx, row in df.iterrows():
        ...
        temp_data.append({
            ...
            'bands': bands,
            'position': get_position_from_price(row['현재가'], bands)
        })
        time.sleep(0.1)
