...
############################################################
# FnGuide 팝업에서 최신월 밴드 주가 5개 추출
############################################################
def get_latest_band_prices_robust(gicode, max_retries=2, delay=0.2):
    """
    FnGuide PBR 밴드 팝업에서 표 전체를 읽어
    가장 최신 날짜 행의 수정주가와 밴드 주가 5개를 반환
    """
    url = (
...
if os.path.exists(file_path):
    ...
    if len(roe_cols) < 3:
        st.error(f"ROE 컬럼이 3개 필요합니다. 현재: {roe_cols}"); st.stop()
